...
$config = Join-Path $workspace 'paper\input\results\llm\v11\open_fed_policy_extraction_v11_predeclared.json'
$output = Join-Path $workspace 'paper\input\results\llm\v11\open_fed_policy_extraction_v11.json'
$scriptFile = Join-Path $workspace 'tools\llm\score_open_fed_policy_v11.py'
...
try {
  while ($true) {
    ...
    Write-Output "V11_START pid=$($script:producer.Id) completed=$($state.success)"
    $thermalTrip=$false
    while (-not $script:producer.HasExited) {
      ...
      Write-Output "V11_THERMAL temp_c=$temperature completed=$($state.success) errors=$($state.errors)"
  ...
  Write-Output "V11_EXTRACTION_COMPLETE output=$output"
}
finally {
