"""Outcome-blind, checkpointed extraction for the frozen SEC RSS v14 corpus."""
...
CORPUS = ROOT / "paper/input/results/llm/v14/sec_rss_archive_corpus_v14/corpus.json"
CANDLE_AUDIT = ROOT / "paper/input/results/llm/v14/binance_vision_btcusdt_4h_candle_v14/candle_artifact_audit.json"
PREDECLARED = ROOT / "paper/input/results/llm/v14/sec_rss_archive_predictive_v14/predeclared.json"
PROMPT = ROOT / "tools/llm/prompts/sec_rss_event_extractor_v14.txt"
...
def preflight(output_dir: Path) -> tuple[list[dict], dict]:
    ...
    candle = json.loads(CANDLE_AUDIT.read_text(encoding="utf-8"))
    ...
    if candle["status"] != "CANDLE_ARTIFACT_AUDIT_PASS_READY_FOR_EXTRACTION":
        raise ValueError("candle artifact has not passed audit")
    ...
    if len(records) != 1120 or len({item["record_id"] for item in records}) != len(records):
        raise ValueError("frozen corpus count or identifiers differ")
    manifest = {
        "schema_version": "sec-rss-extraction-preflight-v14",
        ...
        "inputs": {str(path.relative_to(ROOT)).replace("\\", "/"): sha256(path) for path in (PREDECLARED, CANDLE_AUDIT, CORPUS, PROMPT, SCHEMA, Path(__file__))},
...
def main() -> None:
    ...
    header = {"type": "header", "experiment_id": "sec-rss-archive-llm-only-event-conditioned-v14-development", "manifest": manifest, "run_id": "sec-rss-v14"}
    ...
    for start in range(len(scored), len(records), BATCH_SIZE):
        ...
        try:
            for attempt in range(4):
                try:
                    response = api("/api/chat", {"model": MODEL, "stream": False, "format": schema, "think": False, "keep_alive": "5m", "options": {"temperature": 0, "seed": 20260908}, "messages": [{"role": "system", "content": prompt}, {"role": "user", "content": json.dumps([{ "item_id": item["item_id"], "text": item["headline"]} for item in expected], ensure_ascii=False)}]})
