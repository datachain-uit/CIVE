...
$runner = Join-Path $PSScriptRoot 'resume_cftc_rss_archive_extractor_v15.py'
$output = Join-Path $workspace 'paper\input\results\llm\v15\cftc_rss_archive_predictive_v15\extraction'
...
try {
    while ($true) {
        ...
        $stdout = Join-Path $logs 'cftc_rss_v15.stdout.log'
        $stderr = Join-Path $logs 'cftc_rss_v15.stderr.log'
