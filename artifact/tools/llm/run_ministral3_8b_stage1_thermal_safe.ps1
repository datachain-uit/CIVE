param(
    ...
    [string]$OutputTag = 'v1',
    [int]$ExpectedRecords = 108,
    [int]$SamplePerYear = 12,
...
$modelArtifactStem = 'ministral3_8b'
...
$runs = @(
    (Join-Path $RunRoot "results\llm_daily_reliability_${modelArtifactStem}_run1_${OutputTag}.json"),
    (Join-Path $RunRoot "results\llm_daily_reliability_${modelArtifactStem}_run2_${OutputTag}.json")
)
$audit = Join-Path $RunRoot "results\llm_daily_reliability_${modelArtifactStem}_determinism_${OutputTag}.json"
$gate = Join-Path $RunRoot "results\llm_${candidateId}_challenger_gate_${OutputTag}.json"

foreach ($path in @($RunRoot, $sourceExecutor, $pythonExe, $config, $inputArtifact, $scorerScript)) {
...
function Invoke-ScoringRun {
    ...
    while ($true) {
        ...
        $arguments = @(
            ...
            '--output', $Output,
            '--model', $model,
            '--run-id', "${candidateId}-stage1-run$RunNumber-$OutputTag",
            '--sample-per-year', "$SamplePerYear",
            '--workers', '1',
