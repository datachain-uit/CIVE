"""Outcome-blind, checkpointed extraction for the frozen CFTC RSS v15 corpus."""
...
CORPUS = ROOT / "paper/input/results/llm/v15/cftc_rss_archive_corpus_v15/corpus.json"
CORPUS_GATE = ROOT / "paper/input/results/llm/v15/cftc_rss_archive_corpus_v15/corpus_gate.json"
PREDECLARED = ROOT / "paper/input/results/llm/v15/cftc_rss_archive_predictive_v15/predeclared.json"
...
def preflight(output_dir: Path) -> tuple[list[dict], dict]:
    ...
    if len(records) != 1892 or len({item["record_id"] for item in records}) != len(records):
        raise ValueError("frozen corpus count or identifiers differ")
    manifest = {
        "schema_version": "cftc-rss-extraction-preflight-v15",
...
def main() -> None:
    ...
    header = {"type": "header", "experiment_id": "cftc-rss-archive-llm-only-event-conditioned-v15-development", "manifest": manifest, "run_id": "cftc-rss-v15"}
