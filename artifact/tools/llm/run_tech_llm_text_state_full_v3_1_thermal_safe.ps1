...
$config = Join-Path $runRoot 'configs\tech_llm_text_state_extractor_v3_1_full_predeclared.json'
...
$output = Join-Path $results 'tech_llm_text_state_v3_1_full_development.json'
$gate = Join-Path $results 'tech_llm_text_state_v3_1_full_gate.json'
$canonical = Join-Path $workspace 'paper\input\results\hybrid\hyb003_text_state_full_v3_1'
...
try {
    while ($true) {
        ...
        $arguments = @(
            ...
            '--run-id', 'hyb003-text-state-v3.1-full', '--request-delay-seconds', "$RequestDelaySeconds"
        )
