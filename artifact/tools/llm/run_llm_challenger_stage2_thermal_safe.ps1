...
<#
  Generic Stage 2 runner for any challenger that has passed Stage 1.
  Usage example:
    .\run_llm_challenger_stage2_thermal_safe.ps1 `
        -Model qwen3:8b -CandidateId qwen3_8b -ModelArtifactStem qwen3_8b `
        -Config configs\llm_qwen3_8b_challenger_v1_development.json `
        -Stage1Gate results\llm_qwen3_8b_challenger_gate_v1.json `
        -SeedCheckpoint results\llm_daily_reliability_qwen3_8b_run1_v1.json `
        -DisableThinking
#>
...
$full = "results\llm_daily_full_${ModelArtifactStem}_v1_development.json"
$calibration = "results\llm_outcome_calibration_${ModelArtifactStem}_v1_development.json"
$stage2 = "results\llm_${CandidateId}_challenger_stage2_v1.json"
...
# Preflight checks
foreach ($relative in @($Config, $Stage1Gate)) {
    if (-not (Test-Path -LiteralPath (Join-Path $repo $relative))) {
        throw "Required artifact missing: $relative"
    }
}
$stage1Payload = Get-Content -LiteralPath (Join-Path $repo $Stage1Gate) -Raw | ConvertFrom-Json
if (-not $stage1Payload.passed) { throw 'Stage 1 did not authorize Stage 2' }

# SeedCheckpoint is optional (run1 artifact from Stage 1)
if ($SeedCheckpoint -and -not (Test-Path -LiteralPath (Join-Path $repo $SeedCheckpoint))) {
    throw "Seed checkpoint missing: $SeedCheckpoint"
}
...
try {
    ...
    while ($true) {
        ...
        $arguments = @(
            ...
            '--output', $full,
            '--model', $model,
            '--run-id', "${CandidateId}-full-stage2-v1",
            '--workers', '1',
        ...
        if ($SeedCheckpoint) {
            $arguments += '--seed-checkpoint', $SeedCheckpoint
        }
    ...
    Unload-LocalModel
    Set-BalancedPowerPlan
    Write-Output "FULL_INFERENCE_VALIDATED candidate=$CandidateId records=2927 success=2927 errors=0 unique=2927"
    ...
    Invoke-PythonStep 'stage2-gate-evaluation' @(
        ...
        '--stage1', $Stage1Gate, '--full-run', $full, '--calibration', $calibration,
        '--output', $stage2
    )
    ...
    Write-Output "CHALLENGER_STAGE2_COMPLETE candidate=$CandidateId"
}
finally {
