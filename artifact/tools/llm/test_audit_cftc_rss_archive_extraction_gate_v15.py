...
from audit_cftc_rss_archive_extraction_gate_v15 import audit
...
class CftcRssV15ExtractionGateTests(unittest.TestCase):
    def payloads(self, minimum: int = 2):
        times = ("2025-01-01T00:00:00+00:00", "2025-01-02T00:00:00+00:00")
        corpus = {"corpus_id": "cftc-rss-internet-archive-v15", "records": [source(str(i), t) for i, t in enumerate(times)]}
        ...
        predeclared = {
            "experiment_id": "test", "corpus": {"id": "cftc-rss-internet-archive-v15", "information_time": "archive_capture_at"},
