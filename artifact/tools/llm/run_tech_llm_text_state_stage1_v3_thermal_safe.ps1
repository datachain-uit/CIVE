...
$config = Join-Path $runRoot 'configs\tech_llm_text_state_extractor_v3_stage1_predeclared.json'
...
$run1 = Join-Path $results 'tech_llm_text_state_v3_stage1_run1.json'
$run2 = Join-Path $results 'tech_llm_text_state_v3_stage1_run2.json'
$gate = Join-Path $results 'tech_llm_text_state_v3_stage1_gate.json'
...
function Invoke-Run([int]$RunNumber, [string]$Output) {
    ...
    while ($true) {
        ...
        $arguments = @(
            ...
            '--run-id', "hyb003-text-stage1-run$RunNumber", '--request-delay-seconds', "$RequestDelaySeconds"
        )
